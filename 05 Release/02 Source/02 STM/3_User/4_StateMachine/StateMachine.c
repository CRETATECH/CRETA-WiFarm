...
#ifdef BUTTON
    //<! Check button 1
    else if(GPIO_LOW == button1Check())
    {
        gButton = BUTTON_1;
        gState = STATE_DEVICE;
    }
    //<! Check button 2
    else if(GPIO_LOW == button2Check())
    {
        gButton = BUTTON_2;
        gState = STATE_DEVICE;
    }
...
void stateSensor(void)
{
    ...
    if(EXIT_SUCCESS == shtReadTemp(&ftemp))
    {
        temp = (uint16_t)(ftemp*100);
        serialSendDeviceStatus(SERIAL_FRAME_DEVICE_TEMP, temp);
    }
    else
...
void stateDevice(void)
{
    ...
    {
        // Wait ZCD
        while(DEVICE_OFF == deviceRead(ZCD));
        // Control device
